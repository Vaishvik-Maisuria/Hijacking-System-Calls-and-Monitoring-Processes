...
MODULE_AUTHOR("Your name here .Vaishvik.");
MODULE_LICENSE("GPL");
...
asmlinkage long interceptor(struct pt_regs reg) {
    ...
    //to call original system call we need original system function from table
    // The table must be unlocked for other future operations
    spin_unlock(&my_table_lock);
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
    ...
    //chech if cmd has the correct arguments
    ...
    if (cmd == REQUEST_SYSCALL_INTERCEPT){
        ...
        if (pid == 0) {
            ...
            if (current_uid() != 0){
                ...
                //  clear the list so that you can
		...
		*/
		...
		//root
			...
			// you are root hence you will be allowed to demonitore all pids
            // start demonitoring everything
            // change the state of the monitored
            if (pid == 0){
                destroy_list(syscall);
            }
...
static int init_function(void) {
    ...
    for(i = 0; i < NR_syscalls; i++){
        ...
        table[i].listcount = 0;
        //initialize the linked list to the tables linked list of pid
