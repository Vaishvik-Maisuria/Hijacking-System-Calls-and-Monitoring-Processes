...
int do_intercept(int syscall, int status) {
	int c = vsyscall_arg(MY_CUSTOM_SYSCALL, 3, REQUEST_SYSCALL_INTERCEPT, syscall, getpid());
	printf("%d and %d \n",c, status);
	test("%d intercept", syscall, c == status);
	return 0;
}


int do_release(int syscall, int status) {
	int c = vsyscall_arg(MY_CUSTOM_SYSCALL, 3,REQUEST_SYSCALL_RELEASE, syscall, getpid());
	printf("%d and %d \n",c, status);
	test("%d release", syscall,  c == status);
	return 0;
}
...
int do_start(int syscall, int pid, int status) {
	...
	int c = vsyscall_arg(MY_CUSTOM_SYSCALL, 3,REQUEST_START_MONITORING, syscall, pid);
	printf("%d and %d \n",c, status);
	test("%d start", syscall,  c == status);
	return 0;
}

int do_stop(int syscall, int pid, int status) {
	int c = vsyscall_arg(MY_CUSTOM_SYSCALL, 3,REQUEST_STOP_MONITORING, syscall, pid);
	printf("%d and %d \n",c, status);
	test("%d stop", syscall,  c == status);
	return 0;
}
